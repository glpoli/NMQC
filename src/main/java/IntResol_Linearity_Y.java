...
public class IntResol_Linearity_Y implements PlugInFilter {
    ...
    public void run(ImageProcessor ip) {
        ...
        for (int i = 0; i < roi.getFloatWidth(); i++) {			// Se recorre la imagen 
        ...
        for (int i = 0; i < nbins; i++) {
            ...
            for (int j = 0; j < peakpos.length - 1; j++) {
                ...
                peakpositions[i][j] = Plotter.peakpos(x1, arr1, false) * vw;
                med = med1;
                x[i][j] = j;
                double tresol = Plotter.resolution(x1, arr1, vw, false);
            ...
            peakpositions[i][peakpos.length - 1] = Plotter.peakpos(xf, arr, false) * vw;
            x[i][peakpos.length - 1] = peakpos.length - 1;
            double tresol = Plotter.resolution(xf, arr, vw, false);
            resol = Math.max(resol, tresol);
            meanresol += tresol;
            countpeaks += 1;
        }
